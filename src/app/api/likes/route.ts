...
export async function GET(req: NextRequest) {
    try {
        ...
        if (type === "podcast") {
            if (settings.podcastLikes === undefined) {
                return Response.json({ isLiked: false })
            }
            let isLiked = false
            settings.podcastLikes.forEach(x => {
    ...
    try {
        ...
        if (type === "podcast") {
            if (res.rows.length == 0) {
                ...
                await conn.query("UPDATE podcasts SET likes = likes + 1 WHERE id = $1", [id])
                return Response.json({ isLiked: true })
            }
            const settings = res.rows[0].settings
            if (settings.podcastLikes === undefined) {
                settings.podcastLikes = [id]
                await conn.query("UPDATE settings SET settings = $1 WHERE userid = $2 ", [JSON.stringify(settings), claims.id])
                await conn.query("UPDATE podcasts SET likes = likes + 1 WHERE id = $1", [id])
                return Response.json({ isLiked: true })
            }
            let isUnliked = false
            settings.podcastLikes.forEach((x: number, n: number) => {
                if (x == Number.parseInt(id)) {
                    settings.podcastLikes.splice(n, 1)
                    conn.query("UPDATE settings SET settings = $1 WHERE userid = $2 ", [JSON.stringify(settings), claims.id])
                    conn.query("UPDATE podcasts SET likes = likes - 1 WHERE id = $1", [id])
                    isUnliked = true
                }
            })
            if (isUnliked) {
            ...
            settings.podcastLikes.push(Number.parseInt(id))
            conn.query("UPDATE settings SET settings = $1 WHERE userid = $2 ", [JSON.stringify(settings), claims.id])
            conn.query("UPDATE podcasts SET likes = likes + 1 WHERE id = $1", [id])
            return Response.json({ isLiked: true })
        }
